...
void initLight() {
    glEnable(GL_LIGHTING);  // Ï°∞Î™Ö ?úÏÑ±??

    // Í¥ëÏõê ?úÏÑ±??
    glEnable(GL_LIGHT0);

    // Í¥ëÏõê ?ÑÏπò ?§Ï†ï
    GLfloat light_position[] = { 0.0, 0.0, 0.0, 1.0 };  // (x, y, z, w) - w??Í¥ëÏõê??Î∞©Ìñ• Í¥ëÏõê?∏Ï? ?ÑÏπò Í¥ëÏõê?∏Ï?Î•??òÌ???
    ...
    // ?¨Ïßà ?§Ï†ï ?úÏÑ±??
    //GL_COLOR_MATERIAL???úÏÑ±?îÌïòÎ©?glColor Î™ÖÎ†π?¥Î°ú ?§Ï†ï???âÏÉÅ??Î¨ºÏ≤¥???¨Ïßà???ÅÏö©
...
void init(void) {
    ...
    glShadeModel(GL_SMOOTH);    //Íµ¨Î°ú ?∞Ïù¥??
    glEnable(GL_DEPTH_TEST); // ÍπäÏù¥Î≤ÑÌçº
...
void initSpheres(int numSpheres) {
    ...
    for (int i = 0; i < numSpheres; ++i) {
        ...
        while (sqrt(sphere.x * sphere.x + sphere.y * sphere.y + sphere.z * sphere.z) < innerRadius) {
            // Î∞òÏ?Î¶ÑÏù¥ innerRadius ?¥ÏÉÅ?∏Ï? ?ïÏù∏?òÍ≥†, ?ÑÎãàÎ©??§Ïãú Î¨¥Ïûë??Ï¢åÌëú ?ùÏÑ±
            sphere.x = static_cast<float>(rand()) / RAND_MAX * 2.0 * outerRadius - outerRadius;
            sphere.y = static_cast<float>(rand()) / RAND_MAX * 2.0 * outerRadius - outerRadius;
            sphere.z = static_cast<float>(rand()) / RAND_MAX * 2.0 * outerRadius - outerRadius;
        }
        ...
        spheres.push_back(sphere);  // Î≤°ÌÑ∞??Ï∂îÍ?
    }
}
...
void drawSpheres() {
    ...
    for (const auto& sphere : spheres) {
        count += 1;
        //glColor4f(sphere.r, sphere.g, sphere.b, sphere.a);  // ?¨Î™Ö???ÅÏö©
...
void draw_sun_moon() {
    ...
    GLfloat sun_light_diffuse[] = { 1.0, 1.0, 0.0, 1.0 };
    GLfloat sun_light_specular[] = { 1.0, 1.0, 0.0, 1.0 };
    GLfloat sun_light_emission[] = { 1.0, 1.0, 0.0, 1.0 };  // Î∞úÍ¥ë ?çÏÑ±
    ...
    GLfloat moon_light_diffuse[] = { 0.8, 0.8, 0.8, 1.0 };
    GLfloat moon_light_specular[] = { 1.0, 1.0, 1.0, 1.0 };
    GLfloat moon_light_emission[] = { 0.8, 0.8, 0.8, 1.0 };  // Î∞úÍ¥ë ?çÏÑ±
    ...
    glLightfv(GL_LIGHT1, GL_EMISSION, sun_light_emission);  // Î∞úÍ¥ë ?çÏÑ±
    ...
    glLightfv(GL_LIGHT2, GL_EMISSION, moon_light_emission);  // Î∞úÍ¥ë ?çÏÑ±
...
void drawRay() {
    ...
    // Skybox??Í≤ΩÍ≥Ñ ?ÅÏûê (?? -40, -40, -40?êÏÑú 40, 40, 40)
...
struct Star {
    float x, y, z;   // ?ÑÏû¨ ?ÑÏπò
    float destX, destZ;  // Î™©Ï†ÅÏßÄ ?ÑÏπò
    float size;      // ?¨Í∏∞
};

std::vector<Star> stars;  // Î≥ÑÎì§??Î™©Î°ù
...
void moveStars() {
    // ?¥Îèô ?çÎèÑ Î∞?Î∞©Ìñ• ?§Ï†ï (?? ?ºÏ†ï???çÎèÑÎ°??¥Îèô)
    float speed = 0.1;
...
void MyDisplay() {
    ...
    glPushMatrix();
        ...
        glRotated(-90, 1, 0, 0); //Ï∂îÍ? hy
        glScalef(0.05f, 0.05f, 0.05f);//Ï∂îÍ? hy
...
void MyTimer2(int Value) {
    ...
    if (weatherTime >= 10) {
        weatherTime = 0;
        //cout << " ÔøΩÎºÆÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ : ÔøΩƒ±ÔøΩ ÔøΩœµÔøΩ " << endl;
...
void MyMouseMove(GLint X, GLint Y)
{
    ...
    if (bMousing)
    {
        ...
        g_fSpinY -= (ptCurrentMousePosit.y - ptLastMousePosit.y);//?úÏûêÎ¶??åÏ†Ñ //Í∞ÅÎèÑ?ºÍ≥† ?ùÍ∞Å?òÎ©¥ ?†ÎìØ
    }
...
int main(int argc, char** argv) {
    ...
    // Íµ¨Ï≤¥ Ï¥àÍ∏∞??
    initSpheres(sphere_num);  // 10Í∞úÏùò Íµ¨Ï≤¥Î•?Ï¥àÍ∏∞?îÌï©?àÎã§.
