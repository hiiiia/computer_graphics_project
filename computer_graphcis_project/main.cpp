...
void MyMouseClick(GLint Button, GLint State, GLint X, GLint Y) {
    if (Button == GLUT_LEFT_BUTTON && State ==
        GLUT_DOWN) {
        //set vec4 by mouse position in normalized
            vec4 p((float)X / (float)windowWidth * 2.0 -
                1, ((float)windowHeight - (float)Y) /
                (float)windowHeight * 2.0 - 1, 0.0, 1.0);
        //convert it to world coordinate
        mat4 N = ortho(-2.f, 2.f, -2.f, 2.f, -0.f, 2.f);
        mat4 V = lookAt(myCamera.eye,
            myCamera.at, myCamera.up);
        mat4 M = N * V;
        mat4 invM = inverse(M);
        vec3 convP = invM * p;
        //add a ball
